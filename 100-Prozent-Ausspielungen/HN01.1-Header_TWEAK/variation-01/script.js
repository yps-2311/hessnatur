...
(function(WATO, window) {
    ...
    var shopInEnglish = window.document.documentElement.getAttribute("lang") === "en";
    ...
    // if(shopInEnglish){
    //     window.document.documentElement.classList.add('kk_language_en');
    // }
    ...
    WATO.elem('.logoContainer img', function(logo){
        if(logo){
            logo[0].setAttribute('src', 
                'https://media.hessnatur.com/kk/' + 
                (shopInEnglish ?
                'english/logo/hessnatur_en.svg' : 
                'global/logo/hessnatur_logo.svg'
                )
            );
        }
    });
