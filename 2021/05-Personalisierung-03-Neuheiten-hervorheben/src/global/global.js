...
(function(WATO){
	...
	WATO.prototype.ps03setSegment = function(){
		...
		try {
			var variation = _self.goProfile('customerType') || "Interessent"; // "Neukunde",

			console.log('variation: ', variation);

			if(variation === "Interessent"){
				// Interessent
				if(!doSomeSegment("32812", 2)){
					doSomeSegment("32812");
				}
			}else if(variation === "Neukunde"){
				// Neukunde
				if(!doSomeSegment("32813", 2)){
					doSomeSegment("32813");
					doSomeSegment("32812", 1);
				}
	...
	// 		_self.elem('.headerWrapper', function(underMenu){
	// 			if(underMenu){
	...
	// 				// _self.qs('[href="/de/NEU"]').insertAdjacentHTML('afterend', 
	// 				// 	'<span class="kk_bubble">'+(sumDamen+sumHerren+sumJunior+sumHome)+'</span>'
	// 				// );
	
	// 				var isSubMenu = _self.qs("#mainNavPrgRedirectionForm ul ul ul .h-text-bold");
	...
	// 					var selectedSubMenu = _self.qsa("li:not(.secondLevel):not(.h-text-decoration-none)", isSubMenu.parentNode.parentNode),
	// 						menu1 = _self.qsa("#kk_neumenu a", underMenu.parentNode),
	// 						menu2 = _self.qs("#kk_neumenu2", underMenu.parentNode),
	...
	// 							selectedSubSub = _self.qs(".h-text-bold", subLabel),
	// 							subCategoryNameBox = _self.qs(".prgRedirLink", subLabel),
	// 							subCategoryNameText = subCategoryNameBox.textContent.replace("ä","ae");
	
	// 						if(selectedSubSub){
	// 							addClass(subLabel, 'kk_selected');
	
	// 							var seoText = _self.qs("#kk-headline, .footerSmoBoxWrapper h1");
								
	// 							_self.qs("label", subLabel).insertAdjacentHTML('beforeend', 
	...
	// 							window.location.href = "https://www.hessnatur.com/de/c/"+_self.qs("input.prgRedirData", thisTarget).getAttribute("value").replace("_nav","");
	// 						});
	...
	// 						_self.qs('li:first-child', menu2).addEventListener('click', function(){
	...
	// 					_self.elem('.breadcrumbs li:last-child[itemprop="itemListElement"] strong:last-child', function(lastBreadcrumbLi){
	// 						if(lastBreadcrumbLi){
	// 							var siteproductCounter = parseInt(lastBreadcrumbLi[0].textContent.split("(")[1].split("A")[0]),
	// 								selectedSubCategory = _self.qs(".sidebarNav--nav .h-text-bold"),
	// 								selectedLabel = _self.qs(".kk_selected .kk_bubble", menu2);
				
	// 							// Keine Filter gesetzt und aktuelle Produktmenge stimmt nicht mit gespeicherter überein
	// 							if(selectedSubCategory && !_self.qs(".filterTags")){
	...
	// 				_self.ajax("widgets.crosssell.info/eps/crosssell/recommendations", function(callbackData){
	...
	// 					_self.elem('#kk_crossprods .kk_cbody', function(cbody){
	// 						if(cbody){
	// 							cbody = cbody[0];
	...
	// 										_self.qs('.kk_cprod:last-child', cbody).addEventListener('click', function(e){
	...
	// 								if(_self.qs(position).parentNode !== cbody.parentNode){
	// 									_self.qs(position).parentNode.insertAdjacentElement('beforebegin', cbody.parentNode);
	// 								}
	// 							} catch (error) {
	// 								// console.log('Error: ', error);
	// 							}
	...
	// 		_self.elem(position, function(wrapperOfAllProducts){
	...
	// 				// Kann es sein dass unser neuer Bereich nicht mehr an der korrekten Position steht.
	// 				// Dieses wird hier behoben, indem es neu plaziert wird.
	// 				var crossprodsBody = _self.qs('#kk_crossprods .kk_cbody');
	// 				_self.elem(function(){
	...
	// 			_self.elem('.opens-right > a.h-text-bold', function(categoryName){
	// 				if(categoryName){
	// 					_self.elem('.kk_cattype', function(cattype){
	// 						if(cattype){
