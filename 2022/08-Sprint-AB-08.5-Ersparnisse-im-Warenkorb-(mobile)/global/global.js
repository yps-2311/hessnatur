...
(function(WATO){
	...
	WATO.prototype.s8_5_GOALS = function(variant){
		var _self = this;
		...
		// Upsell Box click goals
		if(variant === 1 || variant === 2){

			addClickGoal('.kk_close_upsell', 'kk_upsell_close_box');

			addClickGoal('#kk_sizes', 'kk_upsell_change_size');

			addClickGoal('#kk_addToCartButton', 'kk_upsell_add_product', true);

			_self.elem('#kk_upsell_wrapper .productItemColor', function(upsellColors){
				if(upsellColors){
					upsellColors.forEach(function(color){
						color.addEventListener('click', function(){
							goalPush('kk_upsell_click_color', false);
						});
					});
				}
			});
			...
			// TODO: Goal to see if the User has seen the upsell box / Segment if the upsell box should be displayed

			// unten im code umgesetzt
		}
	...
	WATO.prototype.s8_5 = function(variant){
		var _self = this;
		...
		_self.elem('.h-mediumOffset-bottom-inner .button.success', function(bottomCTA) {
			if(bottomCTA) {
				...
				let upsellMultipleSizes = false;
				let upsellMultipleColors = false;
				...
				for(var i=0; i < _productCount; i++) {
					...
					lineArtikel.style.color = '#393939';
				...
				// if(document.cookie.indexOf("kk_upsell_hide") === -1) { // !localStorage.getItem('kk_upsell_hide')
					...
					// Push upsell Conditions Meet Goal/Segment
					if(_youSaved + _additionalDiscountAmount >= 10) {  //&& document.cookie.indexOf("kk_upsell_hide") === -1
						// console.log('Push display upsell conditions meet');
						// goalPush('kk_display_upsell_conditions_met');
						pushSegment('32924');
					}
						...
						if(promo) {
							...
							function getProductData(alternativKey) {
								...
								if(_productsIDs.indexOf(prodUsk) !== -1){
									...
									_self.xhr_get2('https://www.hessnatur.com/de/p/'+prodUsk+'/json', false, function(data){
										...
										if(document.cookie.indexOf("kk_upsell_hide") === -1) {
											...
											if(!data){
												...
												buildSizesHTML = function(_sizes){
													...
													if (_sizeCount > 1){
														upsellMultipleSizes = true;
													} 

													return _sizesHTML;
												};

												// console.log('prod_variation_count',prod_variation_count);

												if (prod_variation_count > 1){
													// console.log('prod_variation_count',prod_variation_count);
													upsellMultipleColors = true;
												} 
											...
											for(var p = 0; p < prod_variation_count; p++) {
												...
												if(thisProd.availabilityIndex !== 100 && thisProd.available) {
													if(init_img_url === '') {
														init_img_url = /*thisProd.modelImageUrl ||*/ thisProd.articleImageUrl.replace('_1.','_7.');
														init_color_text = thisProd.color;
														init_price = thisProd.price;
														strike_price = thisProd.strikePrice;
											...
											if(!isGenerallyAvailable){
											 ...
											 if(variant === 1 || variant === 2) {
												...
												if(strike_price !== 0) {
													...
													'<div class="kk_price_div">'+
														'<div class="row">'+
															'<div class="kk_strike_price strikeValue"><span>'+float2Price(strike_price)+'</span>&nbsp;€*</div>'+
															'<div class="discountPrice column shrink price h-xsmallOffset-bottom-inner"><span id="kk_price">'+float2Price(init_price)+'</span>&nbsp;€*</div>'+
														'</div>'+
														'<p class="h-text-muted">inkl. 19% MwSt.</p>'+
													'</div>';
												}
												...
												//sonderfall für Produkte mit nur einer Farbe/
												
												// console.log('upsellMultipleColors', upsellMultipleColors);

												let reasonAdditionalInfo = '';
												if(upsellMultipleColors) {
													reasonAdditionalInfo = 'Erhältlich in verschiedenen Farben.';
												} else if (upsellMultipleSizes) {
													reasonAdditionalInfo = 'Erhältlich in verschieden Größen.';
												}
															...
															'<div id="kk_upsell_inner" class="row">'+
																...
																'<div id="kk_upsell_right" class="medium-12">'+
																	...
																	'<div class="kk_reason_wrapper">' +
																		...
																		'<p class="kk_reason_text">Fügen Sie '+ gramatikAnpassung(name) +' für nur <b id="kk_price_left">'+float2Price(init_price)+'&nbsp;€</b> hinzu. ' + reasonAdditionalInfo + '</p>'+
																		'</div>' +
																	'</div>' +
										...
										var changeColor = function(e){
											...
											const upsellColorSelection = _self.qs('.kk_color_selection');
											const upsellItemColors = _self.qsa('#kk_upsell_wrapper .js-color-bubbles .productItemColor');

											if(upsellColorSelection && upsellItemColors && upsellItemColors.length <= 1){
												addClass(upsellColorSelection, 'kk-two-color-layout');
												return false;
											}
	
											if(upsellColorSelection && upsellItemColors && upsellItemColors.length <= 2){
												// console.log('Only 2 colors');
												addClass(upsellColorSelection, 'kk-two-color-layout');
											}
										...
										// console.log('key',promo[alternativKey]);
										// console.log('key substring',promo[alternativKey].substring(5,7));
										// console.log('test element', _self.qs('.js-color-bubbles li[data-color="'+promo[alternativKey].substring(5,7)+'"] a'));
										...
										if(variant === 1 || variant === 2){
											...
											// console.log("_self.qs('#kk_sizes')", _self.qs('#kk_sizes'));
											// console.log("_self.qs('#kk_addToCartButton', kk_upsell_wrapper)", _self.qs('#kk_addToCartButton', kk_upsell_wrapper));

											// console.log("_self.qs('#kk_sizes').value,", _self.qs('#kk_sizes').value);
											// console.log("price2Float(_self.qs('#kk_price').textContent)", price2Float(_self.qs('#kk_price').textContent));
											...
											_self.qs('#kk_addToCartButton', kk_upsell_wrapper).addEventListener('click', function(){
												...
												// console.log('testAAA'+ _self.qs('#kk_price'));
												...
												if(_prodId !== '') {
													_self.xhr_post2('https://www.hessnatur.com/de/cart/add', 
													...
													function(){
														...
														setCookie('kk_upsell_hide', 'true', ".hessnatur.com", 3);
											...
											// console.log("_self.qs('.kk_close_upsell')", _self.qs('.kk_close_upsell'));

											try {
												_self.qs('.kk_close_upsell').addEventListener('click', function(){
													setCookie('kk_upsell_hide', 'true', ".hessnatur.com", 3);
													addClass(kk_upsell_wrapper, 'kk-hidden');
				...
				if(_youSaved > 0){
					...
					try {
						...
						// console.log('insert closest error');
						goalPush('kk_8.4_error_goal');
					}
